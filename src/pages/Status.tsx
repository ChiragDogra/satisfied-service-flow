...
export default function Status() {
  ...
  const renderServiceRequest = (r: ServiceRequest) => <Card key={r.id} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 animate-fade-in border-border/50">
      <CardContent className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          <div className="flex-1 space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm">
                ...
                {isAdmin ? <ClickableCustomerName customerName={r.customerName} email={r.email} className="font-medium" /> : <span className="font-medium text-foreground">{r.customerName}</span>}
                <span className="text-muted-foreground">•</span>
                <span className="text-muted-foreground">{r.serviceType}</span>
              </div>
              
              {isAdmin && <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-sm text-muted-foreground">
                  <div className="flex items-center gap-2">
            ...
            {(r.estimatedPrice || r.estimatedCompletionTime || r.diagnosedIssue) && <div className="pt-2 border-t border-border/50">
                <div className="space-y-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    {r.estimatedPrice && <div className="flex items-center gap-2 text-green-600">
                        <DollarSign className="h-4 w-4" />
                        <span className="font-medium">Estimated Cost: ₹{r.estimatedPrice.toFixed(2)}</span>
                      </div>}
                    {r.estimatedCompletionTime && <div className="flex items-center gap-2 text-blue-600">
                        <Clock className="h-4 w-4" />
                        <span className="font-medium">
                          Completion: {r.estimatedCompletionTime.includes('T') ? new Date(r.estimatedCompletionTime).toLocaleString('en-IN', {
                      dateStyle: 'medium',
                      timeStyle: 'short'
                  ...
                  {r.diagnosedIssue && <div className="text-sm">
            ...
            {/* Admin Details */}
            {isAdmin && <div className="pt-2 space-y-2 text-sm">
          ...
          <div className="flex sm:flex-col items-start gap-2">
            {isAdmin ? <Select value={r.status} onValueChange={value => handleStatusUpdate(r.id, value as ServiceRequest['status'])}>
              ...
              </Select> : <Badge variant={getStatusVariant(r.status)} className="text-xs font-semibold px-3 py-1 whitespace-nowrap">
                {r.status}
              </Badge>}
            {isAdmin && <Button variant="outline" size="sm" onClick={() => handleEditEstimates(r)} className="h-7 px-2" title="Edit Service Details">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="mb-4">
                    <div className="relative">
                      ...
                      <Input value={semanticSearch} onChange={e => setSemanticSearch(e.target.value)} placeholder="Search anything: ticket ID, customer name, email, service type, status, date..." className="pl-10 h-10" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Input type="date" value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} className="w-[140px] h-8" placeholder="From Date" />
                      <span className="text-xs text-muted-foreground">to</span>
                      <Input type="date" value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} className="w-[140px] h-8" placeholder="To Date" />
                    </div>
                  ...
                  {query && <div className="mt-4 pt-4 border-t border-border">
                      <div className="flex gap-3">
                        <div className="flex-1">
                          <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <Input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Legacy search: Ticket ID, email, or phone" className="pl-10 h-8" />
                          </div>
                        </div>
            ...
            <>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {(semanticSearch.trim() || filterStatus !== '__all__' || filterServiceType !== '__all__' || filterUser !== '__all__' || filterEmail !== '__all__' || filterDateFrom || filterDateTo) && <Badge variant="outline" className="text-xs">
                            Filtered
                          </Badge>}
                      </div>
                    </div>
                    {filteredAdminRequests.length > 0 ? filteredAdminRequests.map(renderServiceRequest) : <Card className="border-dashed">
                        <CardContent className="flex flex-col items-center justify-center py-12 px-6 text-center">
                          <Package className="h-12 w-12 text-muted-foreground/50 mb-4" />
                          <h3 className="text-lg font-semibold mb-2">No requests found</h3>
                          <p className="text-sm text-muted-foreground">
                            {semanticSearch.trim() ? 'No requests match your search criteria. Try different keywords.' : 'Try adjusting your filters or search criteria'}
                          </p>
                        </CardContent>
                      </Card>}
                  </>}
